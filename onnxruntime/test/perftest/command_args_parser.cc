...
/*static*/ void CommandLineParser::ShowUsage() {
  printf(
      ...
      "\t-e [cpu|cuda|dnnl|tensorrt|openvino|nuphar|dml|acl|rocm|migraphx|ketinpu]: Specifies the provider 'cpu','cuda','dnnl','tensorrt', "
      "'openvino', 'nuphar', 'dml', 'acl', 'nnapi', 'coreml', 'rocm' or 'migraphx', 'ketinpu'. "
      "Default:'cpu'.\n"
  ...
  while ((ch = getopt(argc, argv, ORT_TSTR("b:m:e:r:t:p:x:y:c:d:o:u:i:f:F:AMPIvhsqz"))) != -1) {
    switch (ch) {
      ...
      case 'e':
        ...
        } else if (!CompareCString(optarg, ORT_TSTR("ketinpu"))) {
          test_config.machine_config.provider_type_name = onnxruntime::kKetinpuExecutionProvider;
