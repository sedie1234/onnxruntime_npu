...
#endif
  } else if (provider_name == onnxruntime::kKetinpuExecutionProvider) {
#ifdef USE_KETINPU
    Ort::ThrowOnError(
        OrtSessionOptionsAppendExecutionProvider_Ketinpu(session_options,
                                                     performance_test_config.run_config.enable_cpu_mem_arena ? 1 : 0));
#else
    ORT_THROW("Keti npu is not supported in this build\n");
#endif
