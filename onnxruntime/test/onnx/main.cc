...
void usage() {
  printf(
      ...
      "'openvino', 'nuphar', 'rocm', 'migraphx', 'acl', 'armnn', 'nnapi' or 'coreml', 'ketinpu'. "
      "Default: 'cpu'.\n"
...
int real_main(int argc, char* argv[], Ort::Env& env) {
  ...
  std::vector<std::unique_ptr<ITestCase>> owned_tests;
  {
    ...
    if (enable_ketinpu) {
#ifdef USE_KETINPU
      Ort::ThrowOnError(OrtSessionOptionsAppendExecutionProvider_Ketinpu(sf, enable_cpu_mem_arena ? 1 : 0));
#else
      fprintf(stderr, "Keti NPU is not supported in this build\n");
      return -1;
#endif
    }
    if (enable_rocm) {
