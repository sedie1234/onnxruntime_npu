...
std::unique_ptr<IExecutionProvider> DefaultKetinpuExecutionProvider(bool enable_arena) {
#ifdef USE_KETINPU
  return CreateExecutionProviderFactory_Ketinpu(enable_arena)->CreateProvider();
#else
  ORT_UNUSED_PARAMETER(enable_arena);
  return nullptr;
#endif
